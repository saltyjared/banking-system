...
class BankingSystemImpl(BankingSystem):
    ...
    def __init__(self):
        ...
        # i.e. {payment1 : (account1, 86400001, 20, False)} -> key에 value가 하나 뿐인데 timestamp도 있어야 하는 거 아닌가
        self.payments = {}
    ...
    def merge_accounts(self, timestamp: int, account_id_1: str, account_id_2: str) -> bool:
        ...
        # 3
        # new payments structure : {payment_id : (account_id, timestamp, cashback_amount, status)}
        # get key and value(it is set)
        for payment_id, value in self.payments.items():
            # name each components
            account_id, cashback_timestamp, cashback_amount, status = value
            # if there is a cashback for account_id_2 which is pending,
            if account_id == account_id_2 and not status:
                # change the account_id into account_id_1
                self.payments[payment_id] = (account_id_1, cashback_timestamp, cashback_amount, status)

        # 4 
        self.accounts[account_id_1].extend(self.accounts[account_id_2])
        self.accounts[account_id_1].sort(key=lambda x: x[0])  # sorted by timestamp


        # 5
        last_balance_a1 = self.accounts[account_id_1][-1][3]
        last_balance_a2 = self.accounts[account_id_2][-1][3]
        new_balance = last_balance_a1 + last_balance_a2
        self.accounts[account_id_1].append((timestamp, "Account Merge", last_balance_a2, new_balance))


        # 6 -> already achieved by merging the transaction record
        # transaction을 다 옮겻으니까 괜찮을 거라고 생각함
